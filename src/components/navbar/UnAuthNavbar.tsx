...
import { routes, unauthNavbarRoutes } from "../../routes/paths";
          ...
          {unauthNavbarRoutes
            ...
            .map((route) => (
              <Link
                key={route.path}
                to={route.path}
                className={`${loc.pathname === route.path ? "text-primary" : textColorClass} hover:underline hover:opacity-50 font-medium`}
              >
                {t(route.label)}
              </Link>
            ))}
          <Link
            to={routes.unauth.login.path}
            className={`${loc.pathname === routes.unauth.login.path ? "text-primary" : textColorClass} font-bold hover:underline hover:opacity-50 flex flex-nowrap gap-2`}
          >
            {t(routes.unauth.login.label)}
            <ArrowLongRight />
          </Link>
          ...
          <div className="absolute inset-y-0 right-0 w-full max-w-sm overflow-y-auto bg-white p-6 shadow-xl">
            ...
            <div className="mt-6 flex flex-col gap-2 border-t border-gray-200 pt-6">
              {unauthNavbarRoutes.map((route) => (
                <Link
                  ...
                  to={route.path}
                  onClick={() => setOpen(false)}
                  className={`${loc.pathname === route.path ? "text-primary" : "text-gray-900"} rounded-md px-3 py-2 text-base font-semibold hover:bg-gray-50`}
                >
                  {t(route.label)}
                </Link>
              ))}
            ...
            <div className="mt-6 border-t border-gray-200 pt-6">
              <Link
                to={routes.unauth.login.path}
                onClick={() => setOpen(false)}
                className={`${loc.pathname === routes.unauth.login.path ? "text-primary" : "text-gray-900"} flex items-center gap-2 rounded-md px-3 py-2 text-base font-semibold hover:bg-gray-50`}
              >
                {t(routes.unauth.login.label)}
                <ArrowLongRight />
              </Link>
